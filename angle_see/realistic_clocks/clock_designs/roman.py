...
def draw_roman_clock(ctx, width, height, face_color):
    ...
    # Draw hour numbers (Roman numerals)
    for i, numeral in enumerate(roman_numerals):
        ...
        ctx.set_font_size(radius * 0.13)
        ctx.set_source_rgb(0, 0, 0)
        
        text_extents = ctx.text_extents(numeral)
        ctx.move_to(x - text_extents.width / 2, y + text_extents.height / 2)
        ctx.show_text(numeral)
